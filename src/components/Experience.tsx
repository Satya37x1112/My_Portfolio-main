...
const Experience: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} id="experience" className="py-20 bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 scroll-mt-20 relative overflow-hidden transition-colors duration-300">
          ...
          <div className={`transition-all duration-1000 ${isVisible ? 'animate-fade-in-left' : 'opacity-0 -translate-x-10'}`}>
            ...
            <div className="space-y-6">
              {education.map((edu, index) => (
                <div key={index} className="glass-card p-6 hover:neon-glow-cyan transition-all duration-300 hover-lift animate-scale-in animate-delay-200">
          ...
          <div className={`transition-all duration-1000 ${isVisible ? 'animate-fade-in-right' : 'opacity-0 translate-x-10'}`}>
            ...
            <div className="space-y-6">
              {experience.map((exp, index) => (
                <div key={index} className="glass-card p-6 hover:neon-glow-purple transition-all duration-300 hover-lift animate-scale-in animate-delay-400">
                  <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <div ref={achievementsRef} className="grid md:grid-cols-2 gap-6">
            {achievements.map((achievement, index) => (
              <div 
                key={index} 
                className={`glass-card p-6 hover:neon-glow-cyan transition-all duration-300 hover:transform hover:scale-105 hover-lift hover-glow ${
