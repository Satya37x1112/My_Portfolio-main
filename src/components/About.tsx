...
const About: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} id="about" className="py-20 bg-navy-900 text-white scroll-mt-20" style={{backgroundColor: '#1e3a8a'}}>
        ...
        <div ref={imageRef} className="text-center mb-12">
          <div className="flex justify-center">
            <div className={`relative transition-all duration-1000 ${imageVisible ? 'animate-scale-in' : 'opacity-0 scale-75'}`}>
              ...
              <div className="absolute -bottom-4 -right-4 bg-white text-navy-900 rounded-full p-4 shadow-lg" style={{color: '#1e3a8a'}}>
                <Code size={24} />
              </div>
            </div>
          </div>
          ...
          <div className={`space-y-6 text-white transition-all duration-1000 ${isVisible ? 'animate-fade-in-left' : 'opacity-0 -translate-x-10'}`}>
            ...
            <div ref={highlightsRef} className="grid md:grid-cols-3 gap-6 mt-8">
              {highlights.map((highlight, index) => (
                <div 
                  key={index} 
                  className={`text-center p-6 bg-white bg-opacity-10 backdrop-blur-sm rounded-lg shadow-lg border border-white border-opacity-20 transition-all duration-500 hover:shadow-xl hover:bg-opacity-20 ${
